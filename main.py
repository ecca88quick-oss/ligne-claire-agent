...
from image_similarity_flagger import flag_image, flag_batch
from review_dashboard import run_dashboard
...
def step_prompt_gate(prompt: str) -> str | None:
    ...
    if status == "blocked":
        ...
        new_prompt = result.get("rewritten_prompt", prompt)
...
def step_similarity_check(path: str, batch: bool = False):
    ...
    if batch:
        flag_batch(Path(path))
    else:
        flag_image(Path(path))


def step_review_dashboard():
    print("\n[Schritt 4] Review-Dashboard starten...")
    run_dashboard()
...
def main():
    ...
    parser.add_argument(
        ...
        help="Review-Dashboard direkt starten"
    )
    parser.add_argument(
